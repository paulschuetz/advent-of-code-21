...
fun calculateBasinsProduct(heatmap: Array<Array<Int>>): Int {
    ...
    for (lowPoint in lowPoints) {
        ...
        var neighbours = setOf(lowPoint)

        do {
            // first get all neighbouring "2"s then all "3"s and so on...
            neighbours = neighbours.flatMap { findNeighbouringFloatingPoints(heatmap = heatmap, coordinate = it) }.toSet()
...
fun findNeighbouringFloatingPoints(heatmap: Array<Array<Int>>, coordinate: Coordinate): Set<Coordinate> {
    ...
    // above
    if (coordinate.y != 0 && heatmap[coordinate.y - 1][coordinate.x] != 9 && heatmap[coordinate.y][coordinate.x] + 1 == heatmap[coordinate.y - 1][coordinate.x]) {
        neighbouringFloatingPoints.add(Coordinate(x = coordinate.x, y = coordinate.y - 1))
    }
    // under
    if (coordinate.y != yMaxIndex && heatmap[coordinate.y + 1][coordinate.x] != 9 && heatmap[coordinate.y][coordinate.x] + 1 == heatmap[coordinate.y + 1][coordinate.x]) {
        neighbouringFloatingPoints.add(Coordinate(x = coordinate.x, y = coordinate.y + 1))
    }
    // left
    if (coordinate.x != 0 && heatmap[coordinate.y][coordinate.x - 1] != 9 && heatmap[coordinate.y][coordinate.x] + 1 == heatmap[coordinate.y][coordinate.x - 1]) {
        neighbouringFloatingPoints.add(Coordinate(x = coordinate.x - 1, y = coordinate.y))
    }
    // right
    if (coordinate.x != xMaxIndex && heatmap[coordinate.y][coordinate.x + 1] != 9 && heatmap[coordinate.y][coordinate.x] + 1 == heatmap[coordinate.y][coordinate.x + 1]) {
        neighbouringFloatingPoints.add(Coordinate(x = coordinate.x + 1, y = coordinate.y))
    }
